...
```
jax-morphic
├── baseline_model  // example inference model (python)
...
SCENIC methods notebook. It predicts gene regulatory networks (GRNs) separately
for each cell type, using the pipeline:

| Step | Description |
|------|-------------|
| 1. Load data | Reads single-cell RNA counts, cell type labels, and a curated list of known transcription factors (TFs) |
| 2. Preprocess | Normalizes counts across cells, log-transforms the data, and keeps only the top 10,000 most variable genes to reduce noise |
| 3. GRNBoost2 | For each cell type, uses gradient boosting to score how strongly each TF's expression predicts the expression of every other gene, producing a ranked list of candidate TF → gene links; edges with importance below `Z_SCORE_CUTOFF` (default: 2.0) standard deviations are then discarded |
| 4. SCENIC / RcisTarget | Filters candidate links by checking whether the TF's binding motif appears near the gene's promoter in the genome, keeping only biologically supported regulatory edges |
| 5. Output | Saves all predicted TF → gene edges (with importance scores and cell type labels) to `predictions.csv` |
